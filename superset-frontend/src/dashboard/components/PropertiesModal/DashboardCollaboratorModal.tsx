...
interface DashboardCollaboratorModalProps {
  visible: boolean;
  onClose: () => void;
  dashboardId: number;
}
  ...
  const fetchCollaborators = async () => {
    ...
    try {
      const res = await SupersetClient.get({
        endpoint: `/api/v1/dashboard/${dashboardId}/access-info`, // 确保使用正确的API版本和路径
      });
  ...
  return (
    <>
      ...
      <SearchUserOrRoleModal
        ...
        dashboardId={dashboardId} // 传递 dashboardId
      />
    </>
  );
};

export default DashboardCollaboratorModal;
